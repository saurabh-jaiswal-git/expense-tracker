...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res.data && res.data.token) {
        ...
        // @ts-ignore
        window.location.href = '/'; // For web, or use router.replace('/(tabs)') if using expo-router
      } else {
  ...
  const handleSignup = async () => {
    ...
    setLoading(true);
    try {
      // The backend expects firstName for registration, so use a placeholder
      const res = await axios.post(`${API_BASE_URL}/api/users/register`, { email, password, firstName: 'User' });
